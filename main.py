...
if __name__ == '__main__':
    ...
    parser.add_argument('--pool',       required=False, type=str,   default="TRANS", help='How to pool' )
    ...
    # =========================================================================
    # ====  过拟合 / 正则化超参数  (统一在这里调，不用去各子文件里改！)  ====
    # =========================================================================
    # --- 优化器 ---
    parser.add_argument('--weight_decay', required=False, type=float, default=0.05,
                        help='AdamW weight decay (L2正则)。默认0.01。增大(如0.05/0.1)可以缓解过拟合。')

    # --- Dropout ---
    parser.add_argument('--gpt_dropout', required=False, type=float, default=0.2,
                        help='GPT Decoder各层的dropout概率。原先硬编码为0.0(无dropout！)。建议0.1~0.3。')
    parser.add_argument('--encoder_dropout', required=False, type=float, default=0.3,
                        help='双流编码器 Q-Former 内部的dropout概率。默认0.1。建议0.1~0.3。')

    # --- 每个训练阶段的轮次 ---
    parser.add_argument('--epochs_classify', required=False, type=int, default=10,
                        help='classifying(预训练编码器)阶段的训练轮次。')
    parser.add_argument('--epochs_caption', required=False, type=int, default=15,
                        help='captioning阶段的训练轮次。')
    # spotting阶段沿用 --max_epochs

    # --- 学习率调度器 ---
    parser.add_argument('--lr_tmax_classify', required=False, type=int, default=None,
                        help='CosineAnnealing T_max for classifying阶段。默认=epochs_classify。')
    parser.add_argument('--lr_tmax_caption', required=False, type=int, default=None,
                        help='CosineAnnealing T_max for captioning阶段。默认=epochs_caption。')
    parser.add_argument('--lr_tmax_spotting', required=False, type=int, default=None,
                        help='CosineAnnealing T_max for spotting阶段。默认=max_epochs。')
    # =========================================================================
    ...
    parser.add_argument('--model_type', required=False, type=str,   default="gpt", help='Model type' )
